...
class acf_field_widget extends acf_field {
	...
	function render_field_settings( $field ) {
		...
		*/
		...
		acf_render_field_setting( $field, array(
			...
			'instructions'	=> __('Select the wanted sidebars'),
			'type'			=> 'select',
			'name'			=> 'sidebar',
		...
		*/
		...
		foreach( $widget_ids as $id ) {
		    ...
		    $id_simplified = str_replace( $id_var.'-', '', $id ); 
		 	$choices[$id] = $instance[$id_simplified]['title'] ;
		}
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		wp_register_style( 'acf-input-widget', "{$url}assets/css/input.css", array('acf-input'), $version );
	...
	*/
	...
	function format_value( $value, $post_id, $field ) {
		...
		foreach( $value as $id ) {
			...
			$returnValue[] = array('className' => $className, 'instance' => $instance[$id_simplified]);
		}
...
// initialize
new acf_field_widget( $this->settings );
